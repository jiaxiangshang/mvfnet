...
_prn_dir = os.path.dirname(_cur_dir) # ./
...
def estimate_pose(vertices):
    canonical_vertices = np.load(os.path.join(_prn_dir, 'Data/uv-data/canonical_vertices.npy'))
    #canonical_vertices[:, 2] = canonical_vertices[:, 2] + 1000
...
def estimate_pose_mgc(vertices):
    canonical_vertices = np.load(os.path.join(_prn_dir, 'Data/uv-data/canonical_vertices.npy'))
    canonical_vertices[:, 2] = canonical_vertices[:, 2] + 1000
    P = compute_similarity_transform(vertices, canonical_vertices)
    s, R, t = P2sRt(P) # decompose affine matrix to s, R, t
